...
def process_audio_file(audio_file, output_path, spectrogram_transform, backend=None):
    try:
...
def batch_process_audio(audio_files, output_dir, sample_rate=16000, batch_size=64, n_fft=1024, n_mels=128, ws=512, hl=256, max_workers=None, backend=None):
    output_paths = {}
...
def gen_train_spectograms(backend=None):
    audio_files = []
...
def gen_test_spectograms(backend=None):
...
if __name__ == "__main__":
    ...
    gen_train_spectograms(backend)
    gen_test_spectograms(backend)
